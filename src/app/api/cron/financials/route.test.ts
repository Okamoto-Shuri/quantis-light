...
function get(authorization?: string) {
  ...
  return new NextRequest("http://localhost:3000/api/cron/financials", { headers });
}

describe("GET /api/cron/financials", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("CRON_SECRET", SECRET);
    startIngestionRun.mockResolvedValue({ started: true, runId: 7 });
    executeIngestionRun.mockResolvedValue({ runId: 7, target: "financials", status: "failed", processedCount: 0 });
  });
  ...
  it("正しいシークレットなら、財務だけを1回実行し、完了まで待って結果を返す", async () => {
    const before = Date.now();
    const res = await GET(get(`Bearer ${SECRET}`));
    expect(res.status).toBe(200);
    expect(res.headers.get("cache-control")).toContain("no-store");
    expect(await res.json()).toEqual({
      data: { runs: [{ runId: 7, target: "financials", status: "failed", processedCount: 0 }] },
    });
    expect(startIngestionRun.mock.calls.map((call) => call.slice(1))).toEqual([["financials", "cron"]]);

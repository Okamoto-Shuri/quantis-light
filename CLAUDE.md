...
- **有報（EDINET）の取り込み（Sprint 8）**: target `edinet_reports`（表示「有報」。Sprint 9 から「EDINET」）。取り込みは `lib/ingestion/edinet-reports.ts`、EDINET API は `lib/ingestion/edinet/`
  ...
  - `processed_count` = 本文を処理した書類の数（一覧は数えない）。処理済みの書類は取り直さない（`annual_report_extractions` に行があれば処理済み）。取得の失敗（404・500・ZIP でない・接続できない）は処理済みにせず次の実行で再試行（`details.failedDocuments` に上限 50 件）。抽出の失敗は理由付きで処理済み。**抽出の規則を直して取り直すときは、その書類の `annual_report_extractions` の行を消す**
  ...
  - **キーを設定したときの確認手順**（この環境では EDINET のキーが無く、実 API での取り込みは未確認）: 1) Vercel／`.env` に `EDINET_API_KEY` を設定し、取り込み状況で「設定済み」を確かめる 2) 手動で「有報（EDINET）」を1回実行し、実行履歴の結果と `ingestion_runs.details`（`listDatesFetched`・`documentsListed`・`stoppedReason`・`lastFailedStatus`・`failedDocuments`・`discardedFacts`）を見る 3) 何回か続けて実行し、「有報を取得できた銘柄」が増えること、2回目以降に新しい書類が無ければ処理件数が 0 になることを確かめる 4) 1銘柄の詳細の大株主・役員を EDINET の閲覧ページ（リンク）と見比べる。応答の形が違えば「EDINET の応答の形式が想定と異なります」、書類ごとの失敗は `failedDocuments` に残る

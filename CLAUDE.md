...
Quantis Light（個人用の日本株スクリーナー。仕様は `docs/harness/spec.md`）を、ハーネスでスプリントごとに構築中。Sprint 8（EDINET 有報の取り込み・大株主と役員）まで実装済み。
...
| `pnpm test:db` | DB 込みの結合テスト（`src/**/*.db.test.ts`、`vitest.db.config.mts`）。ローカル Supabase と `.env.local` が必要。外部 API だけを差し替えて、取り込み処理を実際の DB に対して動かす。作った行は、テストのファイルごとの接頭辞（9999x・9Y001〜9Y004・9Z001〜9Z120・9W001〜9W008・9W8xx・9W9xx、書類ID S8TEST・S8SEL・S8DB、性能テストの P0000〜P3999 と 0000Z〜3999Z）だけを消して後片付けされる。株価・財務・有報の結合テストは、銘柄マスタ・EDINET の書類にテスト以外の行が無く、株価の成功の実行・取得済みの開示日が無い DB（`db:reset` 直後）が前提 |
...
- **スクリーニング（Sprint 6）**: 画面 `/screening`（`src/app/(app)/screening/page.tsx`、部品は `components/screening/`）と `GET /api/screening`。どちらも `lib/screening/queries.ts` から DB 関数 `screen_stocks(jsonb)`（security invoker。RLS が効く）を1回呼ぶ。外部 API は呼ばない（`lib/ingestion` を import しない）
  ...
  - AC6.12 の注記（上場から約4年未満は条件①が算出不可）は `components/screening/status-mark.tsx` の `PROVISIONAL_CAGR_NOTE`（スクリーニングと銘柄詳細で共有。フックを使わない部品なので server からも使える）。Sprint 9（F15）で差し替える（AC15.12）
...
- **有報（EDINET）の取り込み（Sprint 8）**: target `edinet_reports`（表示「有報」）。取り込みは `lib/ingestion/edinet-reports.ts`、EDINET API は `lib/ingestion/edinet/`
  ...
  - 流れ: キーの確認（未設定なら外部 API を呼ばずに「EDINET の API キーが設定されていません」で `failed`）→ 銘柄マスタ0件なら失敗 → **書類一覧**（期間 = 実行日（JST）の 450 日前〜実行日。直近7日は毎回・**古い順**、そのほかは未取得の日を新しい順。日付ごとに `save_edinet_document_list` で保存と取得済みの記録を1トランザクション）→ 期限・打ち切り（キーの無効・429/503・3xx・5回連続の失敗）で一覧を取り終えていなければ本文に進まない → **本文**（`edinet_ingestion_state` の `targets` を提出日時の新しい順に取得・抽出し、`save_annual_report_extraction` で1書類ずつ保存。訂正に区画が無いと分かると元の書類が新たに対象になるので、対象が無くなるまで繰り返す）。要求の間隔は前の要求の開始から 1,000ms、期限は 210 秒
